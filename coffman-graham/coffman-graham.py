...
class CoffmanGrahan(object):
    ...
    def run(self):
        ...
        for i in range(len(self.graph.nodes())):
            ...
            for z in A:
                ...
                sorted_nodes = sorted(d, key=lambda x:d[x])
    ...
    def create_timetable(self, num_machines):
        ...
        task_end_time = {}
        while task_order != []:
            if all(task in added_tasks and task_end_time[task] <= current_time for task in self._get_node_predecessors(current_task)):
                ...
                added_tasks.append(current_task)
                # print("Remove", current_task, "from", task_order)
                task_order.remove(current_task)
                if task_order != []:
                    current_task = task_order[0]
                if current_machine + 1 == num_machines:
                    current_time += 1
                current_machine = (current_machine + 1) % num_machines
                
            elif num_of_tries >= len(task_order):
                current_time += 1
                num_of_tries = 1
                current_machine = 0
            else:
                current_task = task_order[num_of_tries]
                num_of_tries += 1
